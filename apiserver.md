...
| 1个月小量计划 | 500元   | 80w     | 7.14折|
| 1个月中量计划 | 1000元  | 180w     | 6.25折| 
...
|   checkData  | bool |   是否校验笔记的互动数据（赞、评、藏、分享）数量。默认为false <br> - true：服务端会校验并修正小红书返回的假数据，如修正失败，会过滤数据，直接返回错误code：10504 <br> - false：如果不关注互动数据量，可以传false，能大幅提高接口成功率。    | 否|
